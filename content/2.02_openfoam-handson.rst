...
.. code:: bash
 ...
 mpirun -n 6 snappyHexMesh -parallel -overwrite

 # Restore the O.orig to all processors folders
 for dir in processor*; do [ -d "$dir" ] && cp -r 0.orig "$dir/0"; done

 # Writes fields and boundary condition info for each patch
 mpirun -n 6 patchSummary -parallel

 # Run a potential flow solver
 mpirun -n 6 potentialFoam -parallel -writephi

 # Check the mesh quality
 checkMesh  -constant

 # Run the steady-state solver for incompressible flow
 mpirun -n 6 simpleFoam -parallel
